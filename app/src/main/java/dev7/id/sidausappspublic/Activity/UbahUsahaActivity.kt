...
import kotlinx.android.synthetic.main.activity_ubah_usaha.*
...
class UbahUsahaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_ubah_usaha)
    ...
    private fun postUsaha() {
        btnSimpan.setOnClickListener {
            ...
            if (!pj.isEmpty() && !alamat.isEmpty() && !nama.isEmpty() && !hp.isEmpty()) {
                ...
                if (hp.length < 9) {
                    etHp.error = "Masukan Nomor Telpon Minimal 9 Karakter"
                    etHp.requestFocus()
                    return@setOnClickListener
                }

                if (filteredDesa[0].id == 0) {
                    Toast.makeText(this@UbahUsahaActivity, "Desa belum dipilih", Toast.LENGTH_SHORT).show()
                    return@setOnClickListener
                }
                ...
                request.enqueue(object : Callback<Usaha> {
                    override fun onFailure(call: Call<Usaha>, t: Throwable) {
                        ...
                        Toast.makeText(this@UbahUsahaActivity, "Gagal Nmen", Toast.LENGTH_SHORT).show()

                    }

                    override fun onResponse(call: Call<Usaha>, response: Response<Usaha>) {
                        if (response.isSuccessful){
                            Toast.makeText(this@UbahUsahaActivity, "Sukses", Toast.LENGTH_SHORT).show()
                            finish()
                        }else{
                            Toast.makeText(this@UbahUsahaActivity, "Respon Gagal", Toast.LENGTH_SHORT).show()
    ...
    private fun fetchUsaha(id_usaha:Int){
        api.getUsahabyID("Token ${getToken()}", id_usaha).enqueue(object : Callback<Usaha>{
            override fun onFailure(call: Call<Usaha>, t: Throwable) {
                ...
                Toast.makeText(this@UbahUsahaActivity, "gagal update", Toast.LENGTH_SHORT).show()
            }
    ...
    private fun fillForm(){
        ...
        etHp.setText(currentUsaha.telpon)
        etAlamat.setText(currentUsaha.alamat)
    }

    private fun getToken(): String? {
        val SharedPreferences = this@UbahUsahaActivity.getSharedPreferences("USER", Context.MODE_PRIVATE)
    ...
    private fun fetchKecamatan(){
        apiKecamatan.getKecamatan("Token ${getToken()}").enqueue(object : Callback<List<Kecamatan>>{
            override fun onFailure(call: Call<List<Kecamatan>>, t: Throwable) {
                println(t.message)
                Toast.makeText(this@UbahUsahaActivity, "Terjadi kesalahan", Toast.LENGTH_SHORT).show()
            }
    ...
    private fun spinnerKecamatanBehavior(){
        val adapter = ArrayAdapter(this@UbahUsahaActivity, R.layout.support_simple_spinner_dropdown_item, kecamatans)
    ...
    private fun filterDesa(id_kecamatan : Int){
        ...
        filteredDesa = temp
        val adapter = ArrayAdapter(this@UbahUsahaActivity, R.layout.support_simple_spinner_dropdown_item, filteredDesa)
    ...
    private fun spinnerJenisIzinBehavior() {
        val adapter = ArrayAdapter(this@UbahUsahaActivity, R.layout.support_simple_spinner_dropdown_item, jenisIzins)
    ...
    private fun spinnerKepemilikanBehavior() {
        val adapter = ArrayAdapter(this@UbahUsahaActivity, R.layout.support_simple_spinner_dropdown_item, kepemilikans)
